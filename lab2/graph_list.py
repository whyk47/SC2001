...
class GraphList(Graph):
    ...
    def __post_init__(self):
        # theta(E) time to initialize the adjacency list
    ...
    def distance(self, u: int, v: int) -> float:
        # theta(V) time to get the distance from u to v
    ...
    def adj(self, u: int) -> list[int]:
        # This implementation is theta(V), but it would be theta(1) if we returned an iterator instead of a list
        return [v for v, _ in self.adjacency_list.get(u, [])]

    def outbound(self, u: int) -> list[tuple[int, float]]:
        # theta(1) time to get the outbound edges of u
